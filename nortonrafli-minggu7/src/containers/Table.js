...
import {deleteTodo, toggleTodo, setVisibilityFilter} 
...
class Table extends Component { render() {
    ...
    <table style={{ marginTop: "60px" }} className="table table-hover table-dark"> 
        <thead>
            ...
            <tbody>
                ...
                <tr key={todo.id}>
                    <td style={{ textDecoration: todo.completed ? "line-through" : "none" }}>{todo.text} {todo.completed === true ? "(completed)" : ""}
                    </td> 
                    <td>
    ...
    const getVisibleTodos = (todos, filter) => { switch (filter) {
